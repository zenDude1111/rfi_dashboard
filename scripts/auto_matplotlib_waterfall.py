...
def create_smooth_contour_plot(directory, output_directory, sigma=1):
    ...
    for date in sorted(set(re.match(file_pattern, filename).group(1) for filename in os.listdir(directory) if re.match(file_pattern, filename))):
        ...
        # Create a contour plot without specifying vmin and vmax
        contour = plt.contourf(unique_freqs, unique_times, power_grid_smoothed, levels=50, cmap='viridis')
